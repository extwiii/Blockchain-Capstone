...
contract SolnSquareVerifier is MyCustomERC721Token {
        ...
        public
    {
        ...
        require(isUniqueSolution(a, b, c, input), 'Solution already exist');

        // Check if solution is valid
        require(verifierInterface.verifyTx(a, b, c, input), "Solution not valid");

        // Calculate the hash of the solution which will be the index in the mapping
        uint256 indexHash = calculateHashIndexOfSolution(a, b, c, input);
    ...
    (
        ...
        returns (uint256)
    {
        return uint256(keccak256(abi.encodePacked(a, b, c, input)));
    }
}
